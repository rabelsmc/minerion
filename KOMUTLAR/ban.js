...
var prefix = ayarlar.prefix;
  ...
  if (!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send(`Bu komutu kullanabilmek için **Üyeleri Yasakla** iznine sahip olmalısın! <:mesajat:828207455792136192>`);
  ...
  if (!user) return message.channel.send(`Sunucudan yasaklamak istediğiniz kullanıcıyı etiketlemelisiniz; \`${prefix}ban @Melodia Reklam\` `);
  if (user.id === message.author.id) return message.channel.send('Bir kullanıcı belirtmelisin <:mesajat:828207455792136192>');
 ...
 if (!message.guild.member(user).bannable) return message.channel.send(`Bu kişiyi sunucudan yasaklayamıyorum çünkü \`benden daha yüksek bir role sahip\` ya da \`bana gerekli yetkileri vermedin\`.`);
   ...
   if (!message.guild.member(user).bannable) return message.channel.send('Sunucudaki yetkilileri yasaklayamam! <:mesajat:828207455792136192>');
